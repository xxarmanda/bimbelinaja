    ...
    <form action="{{ route('admin.settings.video.update') }}" method="POST" class="mb-20">
        ...
        <div class="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 space-y-8">
            ...
            <div class="grid grid-cols-1 gap-8">
                ...
                <div>
                    <label class="block text-[10px] font-black uppercase text-gray-400 ml-2 mb-2">Judul Besar (Gunakan &lt;br&gt; untuk baris baru & &lt;span class="text-[#FFC107]"&gt; untuk warna kuning)</label>
        ...
        <div class="lg:col-span-1">
            <div class="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-[#006064]/5 border border-gray-50">
                ...
                <form action="{{ route('admin.sliders.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    <div>
                        <label class="block text-[10px] font-black uppercase text-gray-400 ml-2 mb-2">Pilih Gambar</label>
                        <input type="file" name="image" required class="w-full p-4 rounded-2xl border-2 border-dashed border-gray-100 bg-[#F8FAFC] font-bold text-[#006064] text-xs cursor-pointer">
                    </div>
                    <div>
                        <label class="block text-[10px] font-black uppercase text-gray-400 ml-2 mb-2">Judul Slide (Opsional)</label>
                        <input type="text" name="title" placeholder="Misal: Promo Akhir Tahun" class="w-full p-4 rounded-2xl border-2 border-gray-100 bg-[#F8FAFC] font-bold text-[#006064] text-sm">
                    </div>
                    <button type="submit" class="w-full bg-[#006064] text-white font-black py-4 rounded-2xl shadow-lg uppercase tracking-widest text-[10px]">Upload Gambar</button>
                </form>
            </div>
        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="group relative bg-[#F8FAFC] rounded-3xl overflow-hidden border border-gray-100 hover:border-[#006064]/20 transition-all">
                            <div class="aspect-video w-full overflow-hidden bg-gray-200">
                                <img src="{{ asset('storage/' . $slider->image) }}" class="w-full h-full object-cover group-hover:scale-110 transition-all duration-500">
                            </div>
