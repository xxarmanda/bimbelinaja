...
<div class="max-w-4xl mx-auto pb-20">
    ...
    <div class="bg-white rounded-[3rem] shadow-2xl shadow-teal-900/5 p-10 md:p-16 border border-gray-100">
        <form action="{{ route('admin.registration-steps.update', $registrationStep->id) }}" method="POST" enctype="multipart/form-data">
            @csrf 
            @method('PUT') {{-- PENTING: Untuk rute update di Laravel --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div class="space-y-8" x-data="{ photoPreview: null }">
                    <div>
                        <label class="block text-[10px] font-black text-[#006064] uppercase tracking-widest mb-3 italic">Ikon Langkah (PNG/SVG)</label>
                        ...
                        <div class="relative group cursor-pointer" @click.prevent="$refs.photo.click()">
                            <div class="w-full h-64 bg-gray-50 rounded-[2.5rem] border-4 border-dashed border-gray-100 flex items-center justify-center overflow-hidden group-hover:bg-teal-50 transition-all">
                                ...
                                {{-- Tampilkan Ikon Baru (Jika Ada) --}}
                                <template x-if="photoPreview">
                                    <img :src="photoPreview" class="w-full h-full object-contain p-8">
                                </template>

                                {{-- Tampilkan Ikon Lama (Jika Tidak Ada Upload Baru) --}}
                                <template x-if="! photoPreview">
                                    <div class="text-center p-8">
                                        <img src="{{ asset('storage/' . $registrationStep->icon) }}" class="w-32 h-32 object-contain mx-auto mb-4 opacity-50">
                                        <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Klik Untuk Ganti Ikon</p>
                                    </div>
                                </template>
                            </div>
                        </div>
