        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-16">
            <div class="grid lg:grid-cols-12 gap-0">
                <div class="lg:col-span-5 p-8 bg-gray-50 flex items-start justify-center">
                    @if($selectedProgram->image && file_exists(public_path($selectedProgram->image)))
                    <img src="{{ asset($selectedProgram->image) }}" 
                        alt="{{ $selectedProgram->name }}" 
                        class="w-full rounded-xl shadow-lg object-cover aspect-[4/3]">
                @else
                    <div class="w-full aspect-[4/3] bg-gray-200 flex items-center justify-center rounded-xl">
                        <span class="text-gray-400 text-xl">📷 Tidak ada gambar</span>
                    </div>
                @endif
                </div>
