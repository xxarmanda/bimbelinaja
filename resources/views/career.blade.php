                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 text-center">
                    @forelse($benefits as $b)
                        <div class="group p-4 reveal">
                            <div class="w-32 h-32 bg-teal-50 rounded-3xl flex items-center justify-center mx-auto mb-8 group-hover:bg-[#FFC107] transition-all duration-500 shadow-xl shadow-teal-900/5 p-5">
                               @if($b->image)
                                <img src="{{ asset($b->image) }}"alt="{{ $b->title }}"class="max-w-full max-h-full object-contain transition-transform duration-300 group-hover:scale-110">
                            @else
