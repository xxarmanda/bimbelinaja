...
<div class="p-6 pb-20">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
            ...
            <p class="text-gray-400 font-bold text-[10px] uppercase tracking-widest mt-3 flex items-center">
                <span class="w-8 h-[2px] bg-[#FFC107] mr-3"></span> Panel Kendali Tenaga Pengajar BimbelinAja
            </p>
        </div>
        <a href="{{ route('admin.tutors.create') }}" class="bg-[#006064] hover:bg-teal-950 text-white px-10 py-5 rounded-[2rem] font-black uppercase text-[10px] tracking-widest transition-all shadow-2xl shadow-teal-900/20 active:scale-95 italic flex items-center">
    ...
    {{-- 1. KONFIGURASI TEKS (DENGAN CONTOH PENGISIAN) --}}
    {{-- PERBAIKAN: Pastikan route mengarah ke settings.tutor.update, bukan video --}}
    <form action="{{ route('admin.settings.tutor.update') }}" method="POST" class="mb-20">
        @csrf @method('PATCH')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            {{-- CONTOH PENGISIAN: TENAGA PENGAJAR --}}
                            <input type="text" name="tutor_label" value="{{ $settings['tutor_label'] ?? '' }}" 
                        ...
                        <div class="space-y-2">
                            ...
                            {{-- CONTOH PENGISIAN: Tutor <span>Profesional</span> Kami --}}
                            <input type="text" name="tutor_title" value="{{ $settings['tutor_title'] ?? '' }}" 
                                   placeholder="Contoh: Tutor <span>Profesional</span> Kami"
                                   class="w-full bg-gray-50 border-2 border-transparent focus:border-[#FFC107] focus:bg-white rounded-2xl p-5 font-bold text-sm text-[#006064] outline-none transition-all shadow-inner">
                        </div>
                    </div>
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest ml-3 italic">Deskripsi Seksi Tutor</label>
                        {{-- CONTOH PENGISIAN: Tuliskan alasan mengapa tutor di BimbelinAja sangat berkualitas --}}
                        <textarea name="tutor_desc" rows="6" 
                                  placeholder="Contoh: Kami menghadirkan tenaga pengajar lulusan universitas terbaik yang siap membimbing siswa dengan metode belajar yang seru dan mudah dipahami."
                                  class="w-full bg-gray-50 border-2 border-transparent focus:border-[#FFC107] focus:bg-white rounded-[2.5rem] p-6 font-bold text-sm text-[#006064] outline-none transition-all shadow-inner italic leading-relaxed">{{ $settings['tutor_desc'] ?? '' }}</textarea>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div class="bg-white rounded-[3.5rem] p-10 shadow-sm border border-gray-100 flex flex-col items-center text-center group hover:shadow-2xl transition-all duration-500 relative overflow-hidden">
            ...
            {{-- Foto Tutor (Memperbaiki jalur folder uploads) --}}
            <div class="relative w-36 h-36 mb-8">
                ...
                <div class="relative w-full h-full rounded-full overflow-hidden border-4 border-white shadow-xl z-10 group-hover:scale-105 transition-transform duration-700 bg-gray-50">
                    {{-- PERBAIKAN: Hapus 'storage/' karena file ada di folder public/uploads/tutors --}}
                    @if($tutor->photo && file_exists(public_path($tutor->photo)))
                        <img src="{{ asset($tutor->photo) }}" class="w-full h-full object-cover transition-all duration-700 grayscale group-hover:grayscale-0">
                    @else
            ...
            {{-- TOMBOL AKSI --}}
            <div class="flex items-center space-x-3 w-full pt-6 border-t border-gray-50 relative z-10">
                ...
                <form action="{{ route('admin.tutors.destroy', $tutor->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus tutor {{ $tutor->name }}?')" class="flex-1">
                    @csrf @method('DELETE')
