                        ...
                        <div class="bg-white border border-gray-50 rounded-[3.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group relative overflow-hidden">
                            ...
                            <div class="flex items-start justify-between mb-8 relative z-10">
                                ...
                                <div class="w-20 h-20 bg-gray-50 rounded-[2rem] flex items-center justify-center p-5 group-hover:bg-[#FFC107]/10 transition-all duration-500">
                                    @if($item->image && file_exists(public_path($item->image)))
                                        <img src="{{ asset($item->image) }}" class="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500" alt="{{ $item->name }}">
                                    @else
                                        <span class="text-3xl transition-transform duration-500 group-hover:scale-110">📖</span>
                                    @endif
                                </div>
                                
                                {{-- Tombol Aksi (Hanya Hapus) --}}
                                <div class="flex space-x-2">
                                    <form action="{{ route('admin.sub-programs.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus permanen materi {{ $item->name }}?')">
                                        @csrf @method('DELETE')
                            ...
                            <div class="relative z-10">
                                ...
                                <div class="flex items-center text-gray-300 text-[9px] font-black uppercase tracking-[0.2em] border-t border-gray-50 pt-4">
                                    <svg class="w-3 h-3 mr-2 text-teal-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
